...
function addNumber(num) {
    ...
    if (display.value === '0' || display.value === '') {
        display.value = num; // หากยังไม่มีค่าหรือเป็น 0 ให้แสดงค่าที่กด
    } else {
        display.value += num; // ถ้ามีค่าอยู่แล้ว ให้เพิ่มหมายเลขที่กดเข้าไป
    }
}

function deleteLastChar() {
    const display = document.getElementById('display');
    display.value = display.value.slice(0, -1) || '0'; // ลบตัวสุดท้ายออก หากไม่มีค่าก็ให้แสดง 0
}

function handleAction() {
    const display = document.getElementById('display');
    // ทำการกระทำต่างๆ ที่ต้องการเมื่อกดปุ่ม "เต็ม"
    console.log('ผลรวม: ', display.value); // ตัวอย่างการแสดงผลใน Console
    // คุณสามารถเพิ่มโค้ดที่ต้องการทำได้ที่นี่
}
    ...
    document.getElementById('paymentMethod').addEventListener('change', function() {
        ...
        if (selectedValue == '2') { // 2 คือ paymethod_id ของ PromtPay
            ...
            var totalPrice = calculateTotalPrice(); // คำนวณราคารวมเพื่อแสดง
            document.getElementById('display').value = '0'; // แสดงราคารวมในเครื่องคิดเลข
        } else {
            qrCodeContainer.style.display = 'none'; // ซ่อน QR Code
            calContainer.style.display = 'none'; // ซ่อน Calculator Container
        }
