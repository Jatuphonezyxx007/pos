...
document.getElementById("calculateButton").addEventListener("click", function() {
    ...
    if (change < 0) {
        ...
        display.value = change.toFixed(2); // แสดงเงินทอนในจอ
    }
});
...
document.getElementById('paymentMethod').addEventListener('change', function() {
    ...
    if (selectedValue == '2') {
        var totalPrice = calculateTotalPrice().toFixed(2);
        ...
        function calculate(btnValue) {
            ...
            if (btnValue === "full") {
                output = totalPrice.toString();
            } else if (btnValue === "DEL") {
                output = output.toString().slice(0, -1);
                if (output === "") output = "0";
            } else {
                if (output === "0" && btnValue !== ".") {
                    output = btnValue;
                } else if (btnValue === "." && output.includes(".")) {
                    return;
                } else {
                    output += btnValue;
                }
        ...
        document.getElementById("calculateButton").addEventListener("click", function() {
            const totalPrice = calculateTotalPrice();
            const receivedAmount = parseFloat(display.value);
            ...
            if (change < 0) {
                alert("จำนวนเงินที่ได้รับไม่เพียงพอ");
                return;
            }
