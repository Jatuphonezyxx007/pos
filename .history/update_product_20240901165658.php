...
<div class="col-12 col-sm-8 col-md-12">
  ...
  if (isset($data['id'])) {
    ...
    $id = $data['id'];
    $query = "SELECT size_name FROM size WHERE product_id = $id";
    $result = mysqli_query($conn, $query);
    $firstSizeDisplayed = false;
    ...
    if (mysqli_num_rows($result) > 0) {
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        <?php
        // ตรวจสอบว่าเป็นปุ่มแรกหรือไม่ ถ้าใช่ ให้แสดงลิงก์ "+ เพิ่มขนาด"
        if (!$firstSizeDisplayed) {
          $firstSizeDisplayed = true;
          ?>
          <div class="col-3">
            <a href="add_size.php?product_id=<?= $data['id']; ?>" class="btn btn-primary w-100">+ เพิ่มขนาด</a>
          </div>
          <?php
        }
      }
