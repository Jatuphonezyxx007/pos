...
if (isset($_GET['id'])) {
  ...
  $stmt->bind_param("i", $id); // กำหนดชนิดของ parameter (i = integer)
  $stmt->execute();

  // ดำเนินการคำสั่ง SQL
  $rs = $stmt->get_result(); // ใช้ get_result() เพื่อดึงผลลัพธ์

  if ($rs->num_rows > 0) {
      $data = $rs->fetch_assoc(); // ดึงข้อมูลที่ได้จากการ query
  } else {
      echo "No product found"; // แสดงข้อความเมื่อไม่พบข้อมูล
  }
    ...
    <div class="page-header-title border-bottom pb-2 mb-2 d-flex align-items-center">
        ...
        <div class="ms-auto form-check form-switch">
            <input class="form-check-input" type="checkbox" id="statusSwitch" 
                   ...
                   <?php echo ($productStatus === 'active') ? 'checked' : ''; ?>>
            <label class="form-check-label" for="statusSwitch">Active/Inactive</label>
        </div>
    </div>
</div>
