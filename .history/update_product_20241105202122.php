...
function addSize() {
  ...
  // สร้างฟอร์มขนาดสินค้าใหม่
  const sizeForm = document.createElement('div');
  sizeForm.classList.add('row', 'mb-3');
  sizeForm.id = `sizeForm-${sizeCount}`; // เพิ่ม ID เพื่อระบุฟอร์ม
  ...
  sizeForm.innerHTML = `
    ...
    <div class="col-2 d-flex align-items-center justify-content-center">
      <button type="button" class="btn btn-danger" onclick="removeSize(${sizeCount})">
        <i class="ph ph-trash"></i>
      </button>
    </div>
  `;

  sizeContainer.appendChild(sizeForm);
}

function removeSize(id) {
  const sizeForm = document.getElementById(`sizeForm-${id}`);
  if (sizeForm) {
    sizeForm.remove();
  }
}
...
function saveSizes() {
  // ตรวจสอบข้อมูลที่กรอกก่อนบันทึก
  console.log('บันทึกข้อมูล');
  // ฟังก์ชันนี้สามารถใช้ในการส่งข้อมูลไปที่เซิร์ฟเวอร์
}
