...
<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" onclick="window.location.reload();"></button>
    ...
    <div class="row g-2 mt-2">
        ...
        <div class="col-3">
            <button class="btn btn-success w-100" data-value="=">เต็ม</button>
        </div>
    </div>
</div>
...
// ตัวแปรสำหรับเก็บยอดเงินที่ผู้ใช้ชำระ
let totalPaid = 0; // ตัวแปรเก็บเงินที่จ่ายทั้งหมด

// ฟังก์ชันคำนวณจากการกดปุ่ม 1000, 500, 100
const calculate = (btnValue) => {
    display.focus();

    if (btnValue === "=") {
        // กดปุ่ม 'เต็ม' เพื่อแสดงราคาที่ต้องจ่ายเต็ม
        let totalPrice = calculateTotalPrice().toFixed(2);
        output = totalPrice;
        totalPaid = 0; // รีเซ็ตยอดชำระเมื่อต้องจ่ายเต็ม
    } else if (btnValue === "AC") {
        output = ""; // รีเซ็ตหน้าจอและยอดชำระ
        totalPaid = 0;
    } else if (btnValue === "DEL") {
        output = output.toString().slice(0, -1);
    } else if (btnValue === "100" || btnValue === "500" || btnValue === "1000") {
        // กดปุ่ม 100, 500, 1000 เพื่อเพิ่มยอดชำระ
        totalPaid += parseInt(btnValue);
        output = totalPaid;
        let totalPrice = calculateTotalPrice().toFixed(2);
        
        // คำนวณเงินทอน ถ้าผู้ใช้ชำระมากกว่าราคารวม
        if (totalPaid > totalPrice) {
            let change = totalPaid - totalPrice;
            output += ` (เงินทอน: ${change.toFixed(2)})`; // แสดงเงินทอนบนหน้าจอ
        }
    } else {
        // เพิ่มปุ่มอื่น ๆ เข้ารวม output
...
// เพิ่ม event listener ให้ปุ่มทุกปุ่ม
