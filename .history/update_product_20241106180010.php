...
for ($i = 0; $i < count($size_name); $i++) {
  $size_id = $size_id[$i]; // ดึงค่า size_id จาก hidden input
  $name = $size_name[$i];
  $qty = $size_qty[$i];
  ...
  if (!empty($size_id)) {
      // ถ้ามี size_id แสดงว่าขนาดสินค้านี้มีอยู่แล้วในระบบ ทำการอัปเดตข้อมูล
      $update_sql = "UPDATE size SET qty = ?, re_stock = ?, price = ?, size_name = ? WHERE size_id = ?";
      $stmt_update = $conn->prepare($update_sql);
      $stmt_update->bind_param("iid", $qty, $restock, $price, $name, $size_id);
      ...
      // ถ้าไม่มี size_id แสดงว่าเป็นข้อมูลใหม่ ทำการเพิ่มข้อมูลใหม่
      $insert_sql = "INSERT INTO size (id, size_name, qty, re_stock, price) VALUES (?, ?, ?, ?, ?)";
      $stmt_insert = $conn->prepare($insert_sql);
      $stmt_insert->bind_param("isiii", $product_id, $name, $qty, $restock, $price);
...
<div class="col-md-8">
  ...
  <?php do { ?>
    <div class="row mb-3 size-row">
      ...
      <input type="hidden" name="size_id[]" value="<?= $productData['size_id']; ?>">

      <div class="col-3">
        <div class="form-floating">
          <input type="text" name="size_name[]" class="form-control" placeholder="ชื่อขนาด" value="<?= htmlspecialchars($productData['size_name']); ?>" disabled required>
      ...
      <div class="col-2">
        <div class="form-floating">
          <input type="number" name="size_qty[]" class="form-control" placeholder="จำนวน" value="<?= htmlspecialchars($productData['qty']); ?>" disabled required>
      ...
      <div class="col-2">
        <div class="form-floating">
          <input type="number" name="size_restock[]" class="form-control" placeholder="จุดรีสต๊อก" value="<?= htmlspecialchars($productData['re_stock']); ?>" disabled required>
      ...
      <div class="col-3">
        <div class="form-floating">
          <input type="text" name="size_price[]" class="form-control" placeholder="ราคารวมภาษี" value="<?= htmlspecialchars($productData['price']); ?>" disabled required>
      ...
      <div class="col-2 d-flex align-items-center justify-content-center">
        <!-- ปุ่มแก้ไข -->
        <button type="button" class="btn btn-primary edit-btn">แก้ไข</button>
      </div>
    </div>
  <?php } while ($productData = mysqli_fetch_array($rs)); ?>  

  <!-- ปุ่มสำหรับเพิ่มขนาดสินค้าใหม่ -->
<div class="col-md">
...
document.getElementById('okButton').addEventListener('click', function() {
  ...
  // ฟังก์ชันสำหรับเปิดการแก้ไขแถวที่มีอยู่แล้ว
  document.querySelectorAll('.edit-btn').forEach(button => {
    button.addEventListener('click', function() {
      ...
      // เปลี่ยนปุ่มจาก "แก้ไข" เป็น "ลบ"
      button.textContent = 'ลบ';
      button.classList.remove('btn-primary');
      button.classList.add('btn-danger', 'remove-btn');

      // ตั้งค่า event สำหรับปุ่มลบ
      button.addEventListener('click', function() {
        row.remove();
      }, { once: true });
    });
