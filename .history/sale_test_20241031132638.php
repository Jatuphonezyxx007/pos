...
<div class="modal" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">เงินทอน</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
...
document.getElementById('paymentMethod').addEventListener('change', function() {
    ...
    if (selectedValue == '2') { // 2 คือ paymethod_id ของ PromtPay
        ...
        const calculate = (btnValue) => {
            display.focus();
            if (btnValue === "full") {
                output = totalPrice; // แสดงราคาเต็ม
            } else if (btnValue === "AC") {
                output = "";
            } else if (btnValue === "DEL") {
                output = output.toString().slice(0, -1);
            } else if (["1000", "500", "100"].includes(btnValue)) {
                output = parseFloat(output) + parseFloat(btnValue); // เพิ่มมูลค่าเมื่อกดปุ่ม 1000, 500 และ 100
            } else {
        ...
        // ฟังก์ชันสำหรับแสดงผลและคำนวณเงินทอน
document.addEventListener("DOMContentLoaded", function() {
    ...
    buttons.forEach(button => {
        button.addEventListener("click", function() {
            ...
            if (value === "DEL") {
                ...
                let change = parseFloat(inputAmount) - totalPrice;
                display.value = change >= 0 ? change.toFixed(2) : "จำนวนเงินไม่พอ";
                inputAmount = ""; // reset after calculation
            } else {
                inputAmount += value;
            }
            display.value = inputAmount || "0";
        });
    });
    ...
    document.getElementById("calculateBtn").addEventListener("click", function() {
        let totalPrice = calculateTotalPrice();
        let change = parseFloat(inputAmount) - totalPrice;
        display.value = change >= 0 ? `เงินทอน: ${change.toFixed(2)}` : "จำนวนเงินไม่พอ";
        inputAmount = ""; // reset after calculation
    });
});
...
document.getElementById("calculateButton").addEventListener("click", function() {
    const display = document.querySelector(".display");
    let totalPrice = calculateTotalPrice();
    let receivedAmount = parseFloat(display.value);
    let change = receivedAmount - totalPrice;
    display.value = change.toFixed(2); // แสดงเงินทอน
});
