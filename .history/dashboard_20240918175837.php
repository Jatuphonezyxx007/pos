...
foreach ($sales as $emp_id => $sales_data) {
    ...
    $total_sales = array_sum($sales_data); // รวมยอดขายของพนักงานแต่ละคน
    if ($total_sales > 0) {
        // เพิ่มข้อมูลยอดขายในเดือนนี้
        $datasets[] = [
            'label' => $employees[$emp_id],  // แสดงชื่อพนักงาน
            'data' => array_values(array_map(function($date) use ($sales_data, $currentDate) {
                // ถ้าข้อมูลเป็นวันที่ปัจจุบันหรือก่อนหน้านี้ ให้ใส่ยอดขาย
                return isset($sales_data[$date]) ? $sales_data[$date] : 0;
            }, $dates)),
            'fill' => false,
            'borderColor' => $colors[array_search($emp_id, array_keys($sales)) % count($colors)],
...
const labels = <?php echo json_encode($dates); ?>;
    ...
    // ฟังก์ชันสำหรับอัปเดตกราฟตามเวลาปัจจุบัน
    function updateChart() {
        const currentTime = new Date();
        const formattedTime = currentTime.getFullYear() + '-' + (currentTime.getMonth() + 1).toString().padStart(2, '0') + '-' + currentTime.getDate().toString().padStart(2, '0');
        
        // อัปเดต labels ให้แสดงวันปัจจุบัน
        if (!labels.includes(formattedTime)) {
            labels.push(formattedTime);
        }

        // อัปเดตข้อมูลใน datasets ตามยอดขายล่าสุด
        datasets.forEach((dataset) => {
            // สมมุติว่าได้รับข้อมูลยอดขายปัจจุบันมา
            const latestSale = Math.floor(Math.random() * 1000); // แทนค่าด้วยการดึงข้อมูลจริง

            dataset.data.push(latestSale);
        });

        myChart.update();
    }

    // อัปเดตกราฟทุกๆ 1 นาที (60000 ms)
