...
if (isset($_GET['id'])) {
  ...
  // Create the SQL query to join the products, size, and type tables
  ...
  $stmt->bind_param("i", $id); // Bind the $id variable as an integer parameter
  $stmt->execute();
  $rs = $stmt->get_result();

  if ($rs) {
      $data = $rs->fetch_array(MYSQLI_ASSOC); // Fetch the data from the query
      $productStatus = $data['status']; // Store the product status in the $productStatus variable
  } else {
      echo "Error in query: " . mysqli_error($conn); // Display an error message if the query fails
  }
  $stmt->close(); // Close the statement
} else {
  echo "No Products available"; // Display a message if the $id parameter is not provided
}
?>
...
<div class="col-md-8">
  <div class="card">
    ...
    <div class="card-body pc-component">
      ...
      <div class="row align-items-center">
    <div class="col-2">
        <p class="text-dark mb-0">Size</p>
    </div>
    <div class="col-10">
        <input name="size_name" type="text" class="form-control" value="<?= $data['size_name']; ?>">
    </div>
    <div class="col-10">
        <input name="size_qty" type="text" class="form-control" value="<?= $data['qty']; ?>">
    </div>
    <div class="col-10">
        <input name="size_restock" type="text" class="form-control" value="<?= $data['re_stock']; ?>">
    </div>
    <div class="col-10">
        <input name="size_price" type="text" class="form-control" value="<?= $data['price']; ?>">
    </div>
    <div class="col-10">
        <button type="button" class="btn btn-success"><i class="ph ph-trash"></i></button>
    </div>
</div>
