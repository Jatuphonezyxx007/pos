...
document.getElementById('paymentMethod').addEventListener('change', function() {
    ...
    if (selectedValue == '2') { // 2 คือ paymethod_id ของ PromtPay
            ...
            if (btnValue === "=") {
                if (output !== "") {
                    try {
                        ...
                        console.error("Calculation error:", error); // log error to console
                        output = "Error"; // แสดงข้อความเมื่อเกิดข้อผิดพลาด
                    }

                    // แสดง totalPrice ใน display เมื่อกด "="
                    const totalPrice = calculateTotalPrice().toFixed(2); // คำนวณราคารวม
                    display.value = totalPrice; // แสดงราคาที่คำนวณได้
                } else {
                    display.value = "0"; // ถ้า output ว่างให้แสดง 0
                }
            } else if (btnValue === "AC") {
        ...
        // คำนวณราคารวมเพื่อแสดงที่ display ตอนเริ่มต้น
