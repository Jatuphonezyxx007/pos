...
<div class="modal fade" id="paymentModal" tabindex="-1" aria-labelledby="paymentModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <div class="card-body">
                        ...
                        <div class="row g-2">
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('7')">7</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('8')">8</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('9')">9</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-info w-100 calculator-btn" onclick="addNumber('1000')">1000</button>
                            </div>
                        </div>

                        <div class="row g-2 mt-1">
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('4')">4</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('5')">5</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('6')">6</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-info w-100 calculator-btn" onclick="addNumber('500')">500</button>
                            </div>
                        </div>

                        <div class="row g-2 mt-1">
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('1')">1</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('2')">2</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('3')">3</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-info w-100 calculator-btn" onclick="addNumber('100')">100</button>
                            </div>
                        </div>

                        <div class="row g-2 mt-1">
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('.')">.</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-outline-secondary w-100 calculator-btn" onclick="addNumber('0')">0</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-danger w-100 calculator-btn" onclick="deleteLastChar()">ลบ</button>
                            </div>
                            <div class="col-3">
                                <button class="btn btn-success w-100 calculator-btn" id="actionButton" onclick="handleAction()">เต็ม</button>
                            </div>
                        </div>
                    </div>
    </div>
...
// ฟังก์ชันเพิ่มตัวเลขลงในช่องแสดงผล
function addToResult(value) {
...
// ฟังก์ชันลบตัวเลขตัวสุดท้าย
function deleteLastChar() {
    const resultInput = document.getElementById('result');
    if (resultInput.value.length === 1) {
        resultInput.value = '0';
    } else {
...
function calculateTotal() {
    ...
    if (!isPaymentMode) {
        ...
        resultInput.value = totalPrice.toFixed(2);
        
        // สร้างหรือปรับปรุง element สำหรับแสดงยอดที่ต้องชำระ
        let totalDisplay = document.getElementById('totalDisplay');
        if (!totalDisplay) {
            totalDisplay = document.createElement('div');
            totalDisplay.id = 'totalDisplay';
            totalDisplay.className = 'text-end text-secondary mt-1';
            resultInput.parentNode.insertBefore(totalDisplay, resultInput.nextSibling);
        }
        totalDisplay.textContent = `ยอดที่ต้องชำระ: ${totalPrice.toFixed(2)} บาท`;
        
        // เปลี่ยนปุ่มเป็นโหมดคำนวณเงินทอน
        const calculateButton = document.querySelector('button.btn-success');
        calculateButton.textContent = 'คำนวณ';
        isPaymentMode = true;
    } else {
        // เมื่อกดปุ่ม "คำนวณ" เพื่อคำนวณเงินทอน
        const payment = parseFloat(resultInput.value);
        const totalPrice = calculateTotalPrice();
        const change = payment - totalPrice;
        
        if (change >= 0) {
            resultInput.value = change.toFixed(2);
            ...
            resultInput.value = 'ยอดเงินไม่พอ';
            setTimeout(() => {
        ...
        // รีเซ็ตกลับไปโหมดปกติ
        const calculateButton = document.querySelector('button.btn-success');
        calculateButton.textContent = 'เต็ม';
        isPaymentMode = false;
    }
}

// ฟังก์ชันเริ่มต้นเครื่องคิดเลข
function initializeCalculator() {
    ...
    // เพิ่ม event listener สำหรับการกดปุ่มบนคีย์บอร์ด
