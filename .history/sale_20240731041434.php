...
// เพิ่มสินค้าลงในตาราง
function addProductToTable(productId, productName, productSize, quantity, price) {
    var tableBody = document.getElementById('cart-table-body');
    var existingRow = tableBody.querySelector('tr[data-product-id="' + productId + '"][data-product-size="' + productSize + '"]');

    if (existingRow) {
        // ถ้ามีสินค้ารายการนี้ในตารางอยู่แล้ว
        var existingQuantity = parseInt(existingRow.querySelector('.quantity').textContent);
        var newQuantity = existingQuantity + quantity;
        existingRow.querySelector('.quantity').textContent = newQuantity;

        var existingPrice = parseFloat(existingRow.querySelector('.price').textContent.replace(/,/g, ''));
        var newPrice = (existingPrice / existingQuantity) * newQuantity;
        existingRow.querySelector('.price').textContent = newPrice.toLocaleString() + ' บาท';
    } else {
        // ถ้าไม่มีสินค้ารายการนี้ในตาราง
        var row = document.createElement('tr');
        row.setAttribute('data-product-id', productId);
        row.setAttribute('data-product-size', productSize);
        row.innerHTML = `
            <td class="text-center">${tableBody.rows.length + 1}</td>
            <td class="text-start">
                ${productName}<br>
                <small class="text-muted">${productSize}</small>
            </td>
            <td class="text-center quantity">${quantity}</td>
            <td class="text-center price">${price.toLocaleString()} บาท</td>
        `;
        tableBody.appendChild(row);
    }
}

// เมื่อคลิกปุ่มเพิ่มสินค้า
document.getElementById('add-product-btn').addEventListener('click', function() {
    var productId = selectedProduct.id;
    var productName = selectedProduct.name;
    var productSize = selectedSize;
    var quantity = parseInt(document.getElementById('quantity').value);
    var price = selectedPrice * quantity;

    addProductToTable(productId, productName, productSize, quantity, price);
});
