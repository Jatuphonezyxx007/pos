...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['barcode'])) {
  ...
  exit; // หยุดการทำงานหลังจากส่งผลลัพธ์
}
...
<div class="col-12 col-sm-8 col-md-12">
  ...
  <div class="col-6">
    <input id="barcodeInput" name="ep_name" type="text" class="form-control" value="<?= $data['barcode']; ?>"> 
    <div id="barcodeMessage" class="mt-2"></div> <!-- แสดงข้อความ -->
  </div> 

  <div class="col-3">
    <a href="#" id="checkBarcodeBtn" class="btn btn-success w-100">ตรวจสอบ</a>
  </div>
</div>
...
document.getElementById('checkBarcodeBtn').addEventListener('click', function(e) {
    e.preventDefault(); // ป้องกันการทำให้หน้าโหลดใหม่

    var barcode = document.getElementById('barcodeInput').value;
    
    // ส่งค่าหมายเลขบาร์โค้ดไปยังเซิร์ฟเวอร์
    fetch('update_product.php', { // ใช้ update.php เพื่อส่งข้อมูล
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: 'barcode=' + encodeURIComponent(barcode) // ส่งข้อมูล barcode
    })
    .then(response => response.json())
    .then(data => {
        var messageDiv = document.getElementById('barcodeMessage');
        if (data.exists) {
            messageDiv.innerHTML = '<span class="text-danger">หมายเลขบาร์โค้ดมีสินค้าแล้ว</span>';
        } else {
            messageDiv.innerHTML = '<span class="text-success">หมายเลขบาร์โค้ดยังไม่มีสินค้า</span>';
        }
    })
