...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  // รับข้อมูลจากฟอร์ม
  $p_name = $_POST['p_name'];
  $product_id = $_GET['id']; // รหัสสินค้าที่มาจาก URL
  ...
  // อัปเดทข้อมูลสินค้า
  $sql_product = "UPDATE products SET p_name='$p_name' WHERE p_id='$product_id'";
  mysqli_query($conn, $sql_product);

  // ลูปผ่านขนาดและอัปเดทข้อมูล
  for ($i = 0; $i < count($size_name); $i++) {
      ...
      // เช็คว่ามีขนาดนี้ในฐานข้อมูลหรือไม่
      $size_query = "SELECT size_id FROM sizes WHERE size_name='$current_size_name' AND product_id='$product_id'";
      $result = mysqli_query($conn, $size_query);

      if (mysqli_num_rows($result) > 0) {
          // ถ้ามีขนาดอยู่แล้ว อัปเดท
          $size_id = mysqli_fetch_assoc($result)['size_id'];
          $update_size = "UPDATE sizes SET size_qty='$current_qty', size_restock='$current_restock', size_price='$current_price' WHERE size_id='$size_id'";
          mysqli_query($conn, $update_size);
      } else {
          // ถ้าไม่มีขนาดนี้ เพิ่มขนาดใหม่
          $insert_size = "INSERT INTO sizes (product_id, size_name, size_qty, size_restock, size_price) VALUES ('$product_id', '$current_size_name', '$current_qty', '$current_restock', '$current_price')";
