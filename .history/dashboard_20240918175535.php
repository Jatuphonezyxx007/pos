...
$currentMonth = date('Y-m'); // เดือนปัจจุบันในรูปแบบ ปี-เดือน เช่น 2024-09
$datasets = [];
$colors = ['rgb(75, 192, 192)', 'rgb(153, 102, 255)', 'rgb(255, 159, 64)', 'rgb(255, 99, 132)', 'rgb(0, 50, 133)', 'rgb(231, 41, 41)'];

foreach ($sales as $emp_id => $sales_data) {
    // เช็คว่ายอดขายที่มีอยู่เป็นของเดือนปัจจุบันหรือไม่
    $total_sales = array_sum(array_filter($sales_data, function($date) use ($currentMonth) {
        return strpos($date, $currentMonth) === 0; // ดึงยอดขายเฉพาะเดือนปัจจุบัน
    }));

    if ($total_sales > 0) {
        $datasets[] = [
            'label' => $employees[$emp_id], // ชื่อพนักงาน
            'data' => array_values(array_map(function($date) use ($sales_data, $currentMonth) {
                return isset($sales_data[$date]) && strpos($date, $currentMonth) === 0 ? $sales_data[$date] : 0;
            }, $dates)),
            'fill' => false,
            'borderColor' => $colors[array_search($emp_id, array_keys($sales)) % count($colors)],
            'tension' => 0.1
