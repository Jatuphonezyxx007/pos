...
<div class="col-12 col-sm-8 col-md-12">
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <div class="page-header-title border-bottom pb-2 mb-2 d-flex align-items-center">
        ...
        <div class="ms-auto">
            <span class="badge <?php echo (isset($productStatus) && $productStatus === 'active') ? 'bg-success' : 'bg-danger'; ?>">
                <?php echo (isset($productStatus) && $productStatus === 'active') ? 'เปิดขาย' : 'ปิดการขาย'; ?>
            </span>
            <button class="btn btn-toggle" onclick="toggleStatus()">
                <?php echo (isset($productStatus) && $productStatus === 'active') ? 'เปลี่ยนเป็นปิดการขาย' : 'เปลี่ยนเป็นเปิดขาย'; ?>
            </button>
        </div>
    </div>
</div>
...
function toggleStatus() {
    // ดำเนินการ AJAX หรือการเปลี่ยนแปลงสถานะในฐานข้อมูล
    // สามารถใช้ fetch API หรือ XMLHttpRequest ในการส่งคำขอไปยัง backend
    const currentStatus = document.querySelector('.badge').innerText === 'เปิดขาย' ? 'inactive' : 'active';

    // ส่งคำขอ AJAX ไปยัง server เพื่อเปลี่ยนสถานะ
    // ตัวอย่าง:
    fetch('update_status.php', {
        method: 'POST',
        body: JSON.stringify({ id: <?php echo $id; ?>, status: currentStatus }),
        headers: {
    ...
    .then(data => {
        if (data.success) {
            location.reload(); // รีเฟรชหน้าเพื่อนำข้อมูลใหม่มาแสดง
        } else {
