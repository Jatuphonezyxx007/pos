...
function calculateTotalPrice() {
    ...
    rows.forEach(row => {
        ...
        totalPrice += (quantity || 0) * (price || 0); // ตรวจสอบให้แน่ใจว่า quantity และ price มีค่า
    });
...
document.getElementById('paymentMethod').addEventListener('change', function() {
    ...
    if (selectedValue == '2') { // 2 คือ paymethod_id ของ PromtPay
        ...
        const specialChars = ["%", "*", "/", "-", "+", "="];
        let output = "";
        let totalPrice = calculateTotalPrice().toFixed(2); // คำนวณราคารวมเพื่อแสดงในเครื่องคิดเลข
        display.value = "0"; // แสดงค่าเริ่มต้นเป็น 0

        // Define function to calculate based on button clicked.
        const calculate = (btnValue) => {
            display.focus();
            if (btnValue === "=") {
                // แสดง totalPrice ใน display เมื่อกด "="
                display.value = totalPrice; // แสดงราคาที่คำนวณได้
            } else if (btnValue === "AC") {
                output = ""; // รีเซ็ต output
                display.value = "0"; // รีเซ็ต display เป็น 0
            } else if (btnValue === "DEL") {
                // ถ้ากด DEL ให้ลบตัวอักษรสุดท้ายออก
                output = output.toString().slice(0, -1);
            } else {
                // ถ้า output ว่างและปุ่มเป็น specialChars จะไม่ทำอะไร
                if (output === "" && specialChars.includes(btnValue)) return;
                output += btnValue; // เพิ่มค่าของปุ่มที่กดลงใน output
            }
            // แสดง output ใน display หากมีการกดปุ่ม
            display.value = output || "0"; // แสดงเป็น 0 ถ้า output ว่าง
        };
        ...
        // แสดงราคารวมใน display เมื่อเลือกวิธีการชำระเงิน
        display.value = totalPrice; // แสดงราคารวมในเครื่องคิดเลข
    } else {
        qrCodeContainer.style.display = 'none'; // ซ่อน QR Code
        calContainer.style.display = 'none'; // ซ่อน Calculator Container
    }
...
document.getElementById('payButton').addEventListener('click', function() {
    var totalPrice = calculateTotalPrice().toFixed(2); // คำนวณราคารวม
    alert(`Total Price: ${totalPrice}`); // แสดงราคาที่คำนวณได้เมื่อกดปุ่มชำระเงิน
});
