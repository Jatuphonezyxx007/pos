...
for ($i = 0; $i < count($size_name); $i++) {
  ...
  // ค้นหา size ตาม id ของสินค้าและ size_name
  $check_sql = "SELECT size_id FROM size WHERE id = ? AND size_name = ?";
  ...
  if ($stmt->num_rows > 0) {
      // ถ้ามี size_name อยู่แล้ว ดึง size_id มาใช้ในการอัปเดต
      $stmt->bind_result($size_id);
      $stmt->fetch();

      // อัปเดตข้อมูลขนาดสินค้าโดยใช้ size_id
      $update_sql = "UPDATE size SET qty = ?, re_stock = ?, price = ? WHERE size_id = ?";
      $stmt_update = $conn->prepare($update_sql);
      $stmt_update->bind_param("iidi", $qty, $restock, $price, $size_id);
