        ...
        if ($result) {
            if (mysqli_num_rows($result) > 0) {
                while ($data = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                    ...
                    // Determine image file extension (assuming '.jpg' here, adjust if needed)
                    $image_ext = 'jpg';
                    $image_path = "assets/images/products_2/{$data['id']}.{$image_ext}";
                    
                    // Check if image file exists
                    if (!file_exists($image_path)) {
                        $image_path = 'assets/images/products_2/default.jpg'; // Default image if not found
                    }
    ?>
                <tr>
                    <td class="text-center"><?=$data['id'];?></td>
                    <td class="text-center">
                        <img src="<?=$image_path;?>" class="card-img-top img-icon" alt="Product Image">
                    </td>                    <td class="text-start"><?=$data['name'];?></td>
