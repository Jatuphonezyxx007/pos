...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (mysqli_query($conn, $sql)) {
      ...
      echo "    setTimeout(function() {";
      echo "      window.location.href = 'your_page.php';"; // เปลี่ยน 'your_page.php' เป็นหน้าที่ต้องการ
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // อัปเดตข้อมูลในตาราง size
    $sql = "UPDATE size SET qty = ?, re_stock = ? WHERE size_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iii", $qty, $re_stock, $size_id);

    if ($stmt->execute()) {
        echo "บันทึกข้อมูลสำเร็จ!";
        // คุณสามารถเปลี่ยนเป็นการรีไดเร็กต์ไปยังหน้าที่ต้องการ
