...
// ตรวจสอบรูปภาพ หากไม่มีรูปภาพให้ใช้ default
if (empty($img)) {
    $img = 'default.jpg'; 
}
...
// ใช้เดือนและปีปัจจุบันถ้าไม่ได้เลือกวันที่
...
$sql = "
    ...
    WHERE MONTH(o.order_date) = $selected_month AND YEAR(o.order_date) = $selected_year";

// เพิ่มเงื่อนไขจากตัวเลือก
...
// จัดกลุ่มและเรียงตามพนักงานและวันที่
...
$total_sales_count = $result_total_sales_count->num_rows > 0 ? $result_total_sales_count->fetch_assoc()['total_sales_count'] : 0;

// ดึงจำนวนสินค้าคงเหลือทั้งหมด
$sql_total_qty = "SELECT SUM(qty) AS total_qty FROM size";
$result_total_qty = $conn->query($sql_total_qty);
$total_qty = $result_total_qty->num_rows > 0 ? $result_total_qty->fetch_assoc()['total_qty'] : 0;

// Query หาพนักงานที่มียอดขายสูงสุดในเดือนปัจจุบัน
$sql_max_sales_emp = "
    SELECT e.emp_id, e.emp_name, SUM(o.order_total) AS total_sales
    FROM orders o
    JOIN employees e ON o.emp_id = e.emp_id
...
// อื่นๆ
// เพิ่มโค้ดต่อไปนี้ตามความต้องการ
?>
