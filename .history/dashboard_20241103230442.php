...
// รับค่าจากฟอร์มว่ามีการกดปุ่มค้นหาหรือไม่
$is_search = isset($_POST['search']);

// ตรวจสอบว่ามีการเลือกวันที่หรือกดปุ่มค้นหาหรือไม่
if (!$is_search && empty($selected_date)) {
    $current_month = date('m');
    $current_year = date('Y');
...
// สร้าง SQL สำหรับดึงข้อมูลยอดขาย
...
if (!$is_search) { // ถ้าไม่ได้กดค้นหา ให้แสดงเฉพาะยอดขายในเดือนปัจจุบัน
  $sql .= " AND MONTH(o.order_date) = $current_month AND YEAR(o.order_date) = $current_year";
} elseif (!empty($selected_date)) { // ถ้ามีการเลือกวันที่
  $sql .= " AND MONTH(o.order_date) = $selected_month AND YEAR(o.order_date) = $selected_year";
}

$sql .= " LEFT JOIN orders_detail od ON o.order_id = od.order_id
  LEFT JOIN products p ON od.p_id = p.id
  WHERE 1=1";
