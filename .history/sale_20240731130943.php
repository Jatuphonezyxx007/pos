...
document.getElementById('paymentButton').addEventListener('click', function() {
  ...
  // ฟังก์ชันในการคำนวณราคารวม
  function calculateTotal() {
    var total = 0;
    var rows = document.querySelectorAll('#productTable tbody tr'); // สมมติว่าใช้ id 'productTable' สำหรับตารางสินค้า

    rows.forEach(function(row) {
      var quantity = parseInt(row.querySelector('.itemQuantity').textContent, 10);
      var price = parseFloat(row.querySelector('.itemPrice').textContent.replace(',', ''), 10);
      var sum = quantity * price;
      total += sum;
    });

    return total.toFixed(2); // ใช้สองตำแหน่งทศนิยม
  }

  document.getElementById('paymentMethod').addEventListener('change', function() {
    var selectedValue = this.value;
    var qrCodeContainer = document.getElementById('qrCodeContainer');
    var qrCodeImage = document.getElementById('promtpayQrCode');
    
    if (selectedValue == '2') { // 2 คือ paymethod_id ของ PromtPay
      var total = calculateTotal();
      qrCodeImage.src = "https://promptpay.io/" + total + ".png"; // สร้าง URL ของ QR Code
