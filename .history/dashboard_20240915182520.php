...
if (!empty($selected_date)) {
    // ยอดขายรายชั่วโมงในวันที่เลือก
    $sql = "SELECT HOUR(o.order_date) AS sale_hour, SUM(o.order_total) AS total_sales
            FROM orders o
  ...
  options: {
    scales: {
      x: {
        title: {
          display: true,
          text: 'เวลา' // แสดงชื่อแกน X
        }
      },
      y: {
        title: {
          display: true,
          text: 'จำนวนเงิน' // แสดงชื่อแกน Y
        },
        beginAtZero: true
      }
    }
