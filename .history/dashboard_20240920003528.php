...
// รับ emp_id, type_id, และ paymethod_id จากฟอร์ม
$selected_emp_id = isset($_POST['emp_id']) ? $_POST['emp_id'] : 0;
$selected_type_id = isset($_POST['type_id']) ? $_POST['type_id'] : 0;
$selected_paymethod_id = isset($_POST['paymethod_id']) ? $_POST['paymethod_id'] : 0;
...
// ถ้าต้องการกรองตามประเภทสินค้า (type_id) หรือวิธีการชำระเงิน (paymethod_id) 
// จะต้อง join ตารางที่เกี่ยวข้องด้วย
if ($selected_type_id != 0) {
    $sql .= "JOIN orders_detail od ON o.order_id = od.order_id 
             JOIN products p ON od.p_id = p.id ";
}
if ($selected_paymethod_id != 0) {
    $sql .= "JOIN payment_methods pm ON o.paymethod_id = pm.paymethod_id ";
}

// เงื่อนไข WHERE สำหรับเดือนปัจจุบัน และเช็คว่ามีการเลือกตัวกรองหรือไม่
$sql .= "WHERE MONTH(o.order_date) = $current_month AND YEAR(o.order_date) = $current_year ";
...
// ถ้าเลือกวิธีการชำระเงิน
if ($selected_paymethod_id != 0) {
    $sql .= "AND o.paymethod_id = $selected_paymethod_id ";
}

// กลุ่มผลลัพธ์ตามพนักงานและวันที่
$sql .= "GROUP BY o.emp_id, formatted_date 
         ORDER BY o.emp_id, formatted_date";

$result = $conn->query($sql);

// ดึงข้อมูลเพื่อนำไปแสดง
        ...
        <div class="col-md-6 col-xl-3">
          <div class="card bg-grd-primary order-card">
            <div class="card-body">
              ...
              <h2 class="text-end text-white"><i class="feather icon-award float-start"></i><span><?php echo number_format($average_sales, decimals:2); ?> บาท</span></h2>
