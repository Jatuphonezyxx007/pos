...
if (isset($_GET['id'])) {
  ...
  if ($rs->num_rows > 0) { 
      $sizes = []; // สร้าง array เพื่อเก็บข้อมูลขนาดสินค้าทั้งหมด
      while ($row = $rs->fetch_array(MYSQLI_ASSOC)) {
          $sizes[] = $row; // เก็บข้อมูลแต่ละ row ใน array
      }
  } else {
      echo "No size data found for this product."; // แสดงข้อความเมื่อไม่พบข้อมูล
  }
  $stmt->close();
} else {
  echo "No product ID found in URL"; // แสดงข้อความเมื่อไม่มี id ใน URL
}
...
<div class="col-md-8">
  <div class="card">
    ...
    <?php endforeach; ?>
<?php else : ?>
    <p>No size data found for this product.</p>
<?php endif; ?>
