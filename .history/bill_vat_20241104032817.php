    ...
    <tbody>
        ...
        while ($data = mysqli_fetch_array($rs, MYSQLI_BOTH)) {
            ...
            $total += $sum;
                ...
                // คำนวณราคาไม่รวมภาษี 7%
    $priceBeforeTax = $data['price'] / 1.07; // ราคาต่อหน่วยก่อนรวมภาษี
    $sumBeforeTax = $sum / 1.07; // ราคารวมก่อนรวมภาษี
    $totalTax += $sumBeforeTax;
        ?>
            <tr>
                <td class="text-center small font-table"><?=$i;?></td>
                <td class="font-table">
                    <?=$data['name'];?><br>
                    <span class="text-muted font-table"><?=$data['size_name'];?></span>
                </td>
                <td class="text-center font-table"><?=$data['item'];?></td>
                <!-- ราคาต่อหน่วยก่อนรวมภาษี -->
                <td class="text-center font-table"><?= number_format($priceBeforeTax, 2); ?></td>
                <!-- ราคารวมก่อนรวมภาษี -->
                <td class="text-center font-table"><?= number_format($sumBeforeTax, 2); ?></td>
        ...
        <?php } ?>
        <tr>
            <td colspan="2" class="text-start font-table">&nbsp;</td>
            <td colspan="2" class="text-center font-table">ราคารวม</td>
            <td class="text-center font-table"><?=number_format($totalTax, 2);?></td>
        </tr>

        <tr>
            <td colspan="2" class="text-start font-table">&nbsp;</td>
            <td colspan="2" class="text-center font-table">ภาษีมูลค่าเพิ่ม (7%)</td>
            <td class="text-center font-table"><strong><?=number_format($total, 2);?></strong></td>
        </tr>

        <tr>
            <td colspan="2" class="text-center font-table"><?= convert_number_to_words($total); ?>บาทถ้วน</td>
            <td colspan="2" class="text-center font-table">ราคารวมภาษีมูลค่าเพิ่ม</td>
            <td class="text-center font-table"><strong><?=number_format($total, 2);?></strong></td>
        </tr>
    </tbody>
</table>
