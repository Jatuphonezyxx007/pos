...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        $date = $row['formatted_date']; // สมมติว่า 'formatted_date' เป็น Datetime
        $formatted_date = date('d/m/y', strtotime($date)); // แปลงวันที่
        $month = date('m', strtotime($date)); // ดึงเดือนจากวันที่
        $year = date('Y', strtotime($date));  // ดึงปีจากวันที่

        // ตรวจสอบว่าข้อมูลอยู่ในเดือนและปีปัจจุบันหรือไม่
...
// Query ดึงบิลที่มียอดขายมากที่สุด และดึงชื่อพนักงานที่เกี่ยวข้อง
$sql_max_order = "
    SELECT o.order_total, o.emp_id, e.emp_name 
    FROM orders o
    JOIN employees e ON o.emp_id = e.emp_id
    ORDER BY o.order_total DESC
    LIMIT 1";  // จำกัดให้ดึงเพียงบิลที่มียอดขายสูงสุด

$result_max_order = $conn->query($sql_max_order);

if ($result_max_order->num_rows > 0) {
    $row_max_order = $result_max_order->fetch_assoc();
    $max_order_total = $row_max_order['order_total'];  // ยอดขายมากที่สุด
    $emp_name = $row_max_order['emp_name'];  // ชื่อพนักงานที่ขายบิลนั้น
    $emp_id = $row_max_order['emp_id'];  // ID พนักงาน
}
        ...
        <div class="col-md-6 col-xl-3">
          <div class="card bg-grd-warning order-card">
            <div class="card-body">
              ...
              <h2 class="text-end text-white"><i class="feather icon-repeat float-start"></i><span><?php echo number_format($max_order_total, decimals:2); ?> บาท</span></h2>
              <p class="m-b-0">พนักงานขาย<span class="float-end">$5,032</span></p>
            </div>
          </div>
        </div>
