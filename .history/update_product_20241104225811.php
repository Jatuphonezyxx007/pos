...
if (isset($_GET['id'])) {
    ...
    $sizes = [];
    $sizeSql = "SELECT * FROM size WHERE product_id = ?";
    $sizeStmt = $conn->prepare($sizeSql);
    $sizeStmt->bind_param("i", $id);
    $sizeStmt->execute();
    $sizeResult = $sizeStmt->get_result();

    while ($sizeData = $sizeResult->fetch_assoc()) {
        $sizes[] = $sizeData;
    }
    $sizeStmt->close();
} else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE size SET qty='$qty', re_stock='$re_stock', price='$price' WHERE size_id='$size_id'";
    
    if (mysqli_query($conn, $sql)) {
        echo "<script>document.addEventListener('DOMContentLoaded', function() { var myModal = new bootstrap.Modal(document.getElementById('exampleModal'), {}); document.getElementById('modalMessage').innerHTML = '<div class=\"text-success\"><i class=\"bi bi-check-circle-fill\"></i> ข้อมูลถูกอัปเดตเรียบร้อยแล้ว</div>'; myModal.show(); setTimeout(function() { window.location.href = 'update_product.php?id=$id'; }, 1000); });</script>";
    } else {
...
<div id="sizeContainer">
    <?php foreach ($sizes as $size): ?>
        <div class="row g-2 mt-3" id="sizeRow<?= $size['size_id'] ?>">
            <div class="col-md-3">
                <input type="text" name="size[<?= $size['size_id'] ?>][size]" class="form-control" value="<?= htmlspecialchars($size['size_name']) ?>" required>
            </div>
            <div class="col-md-2">
                <input type="number" name="size[<?= $size['size_id'] ?>][quantity]" class="form-control" value="<?= $size['qty'] ?>" required>
            </div>
            <div class="col-md-3">
                <input type="number" name="size[<?= $size['size_id'] ?>][restock]" class="form-control" value="<?= $size['re_stock'] ?>" required>
            </div>
            <div class="col-md-3">
                <input type="text" name="size[<?= $size['size_id'] ?>][price]" class="form-control" value="<?= number_format($size['price'], 2) ?>" required oninput="formatPrice(this)">
            </div>
            <div class="col-md-1">
                <button type="button" class="btn btn-danger" onclick="removeSize(<?= $size['size_id'] ?>)">ลบ</button>
            </div>
        </div>
    <?php endforeach; ?>
</div>
