...
// ตรวจสอบว่ามี ID ที่ส่งมาผ่าน URL หรือไม่
if (isset($_GET['id'])) {
  $id = $_GET['id'];

  // สร้าง SQL เพื่อดึงข้อมูลสินค้าตาม ID และข้อมูลขนาดทั้งหมดที่เกี่ยวข้อง
  ...
  if ($rs->num_rows > 0) { 
      // เก็บผลลัพธ์ทั้งหมดในตัวแปร $data
      $data = $rs->fetch_all(MYSQLI_ASSOC);
      
      // แสดงผลข้อมูลที่ดึงมา
      foreach ($data as $item) {
          echo "Product Name: " . htmlspecialchars($item['name']) . "<br>";
          echo "Type: " . htmlspecialchars($item['type_name']) . "<br>";
          echo "Size Name: " . htmlspecialchars($item['size_name']) . "<br>";
          echo "Quantity: " . htmlspecialchars($item['qty']) . "<br>";
          echo "Restock Point: " . htmlspecialchars($item['re_stock']) . "<br>";
          echo "Price: " . htmlspecialchars($item['price']) . "<br><br>";
      }
  } else {
      echo "No product data found for the given ID."; // แจ้งเตือนเมื่อไม่พบข้อมูลที่ตรงกับ ID
  }
  $stmt->close();
} else {
  // ถ้าไม่มี ID ใน URL ทำการดึงข้อมูลสินค้าทั้งหมด
  ...
  if ($rs) {
      ...
      if (mysqli_num_rows($rs) > 0) {
          while ($item = mysqli_fetch_array($rs)) {
              echo "Product Name: " . htmlspecialchars($item['name']) . "<br>";
              echo "Minimum Price: " . htmlspecialchars($item['min_price']) . "<br>";
              echo "Maximum Price: " . htmlspecialchars($item['max_price']) . "<br><br>";
          }
      } else {
          echo "No products available"; // แสดงข้อความเมื่อไม่พบข้อมูลในกรณีที่ไม่มี ID
      }
  } else {
...
<div class="col-md-4">
  <div class="card">
    <div class="card-body pc-component">
      ...
      <div class="pic">
        <img src="assets/images/products_2/<?=$data3['id'];?>.<?=$data3['img'];?>" class="card-img-top rounded mx-auto d-block" alt="">
      </div>
      <br><br><br>
      <div class="col">
  ...
  <div class="card">
    <div class="card-header">
      <div class="row align-items-center">
        ...
        <div class="col-10">
          <input class="form-control" type="text" name="ep_id" placeholder="<?= $data3['id']; ?>" aria-label="Disabled input example" disabled>              
        </div>          
      </div>
    </div>
    <div class="card-body pc-component">
        ...
        <div class="row mb-3">
            <div class="col-4">
                <div class="form-floating">
                    <input type="text" name="size_name" class="form-control" id="size_name" placeholder="ชื่อขนาด" value="<?= htmlspecialchars($item['size_name']); ?>" required>
                    <label for="size_name">ชื่อขนาด</label>
                </div>
            </div>
            <div class="col-2">
                <div class="form-floating">
                    <input type="number" name="size_qty" class="form-control" id="size_qty" placeholder="จำนวน" value="<?= htmlspecialchars($item['qty']); ?>" required>
                    <label for="size_qty">จำนวน</label>
                </div>
            </div>
            <div class="col-2">
                <div class="form-floating">
                    <input type="number" name="size_restock" class="form-control" id="size_restock" placeholder="จุดรีสต๊อก" value="<?= htmlspecialchars($item['re_stock']); ?>" required>
                    <label for="size_restock">จุดรีสต๊อก</label>
                </div>
            </div>
            <div class="col-2">
                <div class="form-floating">
                    <input type="text" name="size_price" class="form-control" id="size_price" placeholder="ราคา" value="<?= htmlspecialchars($item['price']); ?>" required>
