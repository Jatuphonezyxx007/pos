...
<!-- ปุ่มปิด modal -->
<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" id="closeModalButton" onclick="resetCalculator()"></button>
      </div>
...
function resetCalculator() {
    const display = document.querySelector(".display");
    display.value = '0'; // เซ็ตค่า display เป็น 0 เมื่อปิด modal
}

// ฟังก์ชันที่เรียกเมื่อปิด modal
document.getElementById('paymentModal').addEventListener('hide.bs.modal', function() {
    resetCalculator(); // เรียกใช้ฟังก์ชัน resetCalculator
});
