...
$sql_total_sales = "
    SELECT IFNULL(SUM(order_total), 0) AS total_sales
    FROM orders
...
$stmt->execute();
$result_total_sales = $stmt->get_result();

$total_sales = 0; // กำหนดค่าเริ่มต้น
if ($result_total_sales->num_rows > 0) {
    $row_total_sales = $result_total_sales->fetch_assoc();
    $total_sales = $row_total_sales['total_sales'];
}
...
$sql_total_days = "
    ...
    WHERE MONTH(order_date) = ? AND YEAR(order_date) = ?
";
$stmt = $conn->prepare($sql_total_days);
$stmt->bind_param("ii", $current_month, $current_year);
$stmt->execute();
$result_total_days = $stmt->get_result();

$total_days = 0; // กำหนดค่าเริ่มต้น
...
$sql_total_bills = "
    SELECT IFNULL(COUNT(DISTINCT o.order_id), 0) AS total_bills
    FROM orders o
    WHERE MONTH(o.order_date) = ? AND YEAR(o.order_date) = ?
";
$stmt = $conn->prepare($sql_total_bills);
$stmt->bind_param("ii", $current_month, $current_year);
$stmt->execute();
$result = $stmt->get_result();

$total_bills = 0; // กำหนดค่าเริ่มต้น
if ($result->num_rows > 0) {
...
$sql_total_items_sold = "
    SELECT IFNULL(SUM(od.item), 0) AS total_items_sold
    FROM orders_detail od
    JOIN orders o ON od.order_id = o.order_id
    WHERE MONTH(o.order_date) = ? AND YEAR(o.order_date) = ?
";
$stmt = $conn->prepare($sql_total_items_sold);
$stmt->bind_param("ii", $current_month, $current_year);
$stmt->execute();
$result = $stmt->get_result();

$total_items_sold = 0; // กำหนดค่าเริ่มต้น
if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $total_items_sold = $row['total_items_sold'];
}

// ตรวจสอบและตั้งค่า "ยังไม่มี" ถ้าไม่มีสินค้าที่ขาย
if ($total_items_sold == 0) {
    $total_items_sold = "ยังไม่มี";
}

// ตรวจสอบและตั้งค่า "ยังไม่มี" ถ้าไม่มีพนักงานที่ขายได้มากที่สุด (สมมติว่าคุณมีฟังก์ชันสำหรับหาพนักงานขายที่ยอดเยี่ยมที่สุด)
$top_selling_employee = getTopSellingEmployee($current_month, $current_year);
if ($top_selling_employee == null) {
    $top_selling_employee = "ยังไม่มี";
}

// แสดงผล
echo "ยอดขายรวม: " . ($total_sales > 0 ? number_format($total_sales) . " บาท" : "0 บาท") . "<br>";
echo "สินค้าที่ขายในปัจจุบัน: " . $total_items_sold . "<br>";
echo "พนักงานขายที่ขายได้มากที่สุด: " . $top_selling_employee . "<br>";
