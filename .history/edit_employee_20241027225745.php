...
// ตรวจสอบว่ามีการส่ง role_id ผ่าน POST หรือไม่ ถ้าไม่มี ให้ใช้ค่าจากฐานข้อมูล
$role_id = isset($_POST['ep_role']) ? $_POST['ep_role'] : $data['role_id'];
...
// อัปเดตข้อมูลเมื่อมีการส่งฟอร์มแบบ POST
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  // อัปโหลดรูปภาพหากมีการเลือก
  $img_sql = "";
  if (!empty($_FILES['ep_pic']['name'])) {
...
<div class="col-12 col-sm-8 col-md-12">
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <select class="form-select" id="role" aria-label="role" name="ep_role">
        ...
        // ดึงค่า role_id ของพนักงานจาก $data ที่มีอยู่ในฐานข้อมูล
        $emp_role_id = $data['role']; // role_id ปัจจุบันของพนักงาน
        ...
        if ($rs2) {
            while ($data2 = mysqli_fetch_array($rs2)) {
                // ถ้า role_id ตรงกับ $emp_role_id ให้แสดง option นั้นเป็น selected
