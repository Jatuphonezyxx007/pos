...
<!-- [ Header ] end -->
  ...
  <small>
    <?= "วัน" . thai_day($data['order_date']) . " " . date('d', strtotime($data['order_date'])) . " " . thai_month($data['order_date']) . " " . thai_year($data['order_date']) . " " . date('H:i', strtotime($data['order_date'])) . " น."; ?>
  </small>
</td>          <td class="text-end"><?=number_format($data['order_total'], 2);?></td>
