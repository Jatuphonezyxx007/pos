...
// ตรวจสอบให้แน่ใจว่า SELECT มีการใช้งาน SUM และ GROUP BY อย่างถูกต้อง
$sql = "SELECT DATE_FORMAT(o.order_date, '%d-%m-%Y') as formatted_date, SUM(o.order_total) as total_sales 
        FROM orders o
        JOIN order_detail od ON o.order_id = od.order_id
        JOIN products p ON od.p_id = p.product_id
        JOIN type t ON p.type_id = t.type_id
        WHERE 1=1"; // เริ่มจาก WHERE 1=1 เพื่อเติมเงื่อนไขต่อไป

// ตรวจสอบว่ามีการเลือก type_id หรือไม่
if ($selected_type_id != 0) {
    $sql .= " AND t.type_id = $selected_type_id";
}

// ตรวจสอบว่าเลือก emp_id หรือไม่
if ($selected_emp_id != 0) {
    $sql .= " AND o.emp_id = $selected_emp_id";
}

// ตรวจสอบว่าเลือก paymethod_id หรือไม่
...
// สร้าง arrays สำหรับเก็บวันที่และยอดขาย
$dates = [];
$sales = [];
