...
<!-- [ Header ] end -->
  ...
  <tbody>
        ...
        $sql = "SELECT products.*, MIN(size.price) AS min_price, MAX(size.price) AS max_price, type.*
        CASE 
        ...
        WHERE `products`.`id` = `type`.`type_id`
        ...
        //         CASE 
        //             WHEN p.qty > 7 THEN 'พร้อมขาย'
        //             WHEN p.qty > 0 AND p.qty <= 7 THEN 'ใกล้หมด'
        //             ELSE 'สินค้าหมด'
        //         END AS status 
        //         FROM products p 
