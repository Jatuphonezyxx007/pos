  ...
  <div class="row align-items-center">
    ...
    <div class="col-9">
      <select class="form-select" id="role" aria-label="role" name="ep_role">
        ...
        // ดึงค่าที่ต้องการให้ถูกเลือก (อาจมาจากฐานข้อมูลหรือการตั้งค่า)
        $selected_role_id = null; // เริ่มต้นค่าเป็น null หรือกำหนดค่าอื่นตามที่ต้องการ
        
        // สมมุติว่าคุณดึงค่า $selected_role_id จากฐานข้อมูลหรือ session
        // ตัวอย่างการดึงค่า role_id ที่เลือกไว้
        // $query = "SELECT role_id FROM some_table WHERE some_condition";
        ...
        while ($data2 = mysqli_fetch_array($rs2)) {
          // ตรวจสอบว่า option ไหนที่ต้องถูกเลือก
          $selected = ($data2['role_id'] == $selected_role_id) ? 'selected' : '';
        ?>
          <option value="<?=$data2['role_id'];?>" <?=$selected;?>><?=$data2['role_name'];?></option>
        <?php } ?>
      </select>
    </div>          
  </div>
