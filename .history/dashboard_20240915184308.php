...
$employee_names = [];
$employee_sales = [];
if ($result_employee_sales->num_rows > 0) {
    while ($row = $result_employee_sales->fetch_assoc()) {
        $employee_names[] = $row['emp_name'];
...
const labels = <?php echo json_encode($employee_names); ?>;
    const data = {
        labels: labels,
        datasets: [{
            label: 'ยอดขายของพนักงาน',
            data: <?php echo json_encode($employee_sales); ?>,
