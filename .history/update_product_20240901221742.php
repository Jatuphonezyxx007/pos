...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  // ตรวจสอบว่าได้ค่า size_id จากฟอร์มหรือไม่
  if(isset($_POST['size_id'])) {
    $size_id = $_POST['size_id'];
  } else {
    // กรณีที่ไม่พบ size_id ให้แสดงข้อความหรือ error
  ...
  if (mysqli_query($conn, $sql)) {
      ...
      echo "    setTimeout(function() {";
      echo "      window.location.href = 'update_product.php?id=$size_id';"; // เปลี่ยนไปที่หน้า update_product.php พร้อมส่งค่า size_id ที่ถูกต้อง
