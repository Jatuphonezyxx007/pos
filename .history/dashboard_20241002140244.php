...
if (empty($dates)) {
  $dates = ['ยังไม่มีข้อมูล'];
}
if (empty($sales)) {
  $sales[0] = ['ยังไม่มีข้อมูล' => 0];
}
if (empty($employees)) {
  $employees[0] = 'ยังไม่มีพนักงาน';
}
...
// Query เพื่อดึงยอดขายรวมทั้งหมดของเดือนนี้
$sql_total_sales = "SELECT SUM(order_total) AS total_sales FROM orders WHERE MONTH(order_date) = $selected_month AND YEAR(order_date) = $selected_year";
...
// Query เพื่อหาพนักงานที่มียอดขายสูงสุด
$sql_max_sales_emp = "
  SELECT e.emp_name, SUM(o.order_total) AS total_sales
  FROM orders o
  JOIN employees e ON o.emp_id = e.emp_id
