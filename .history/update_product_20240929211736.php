...
// ฟังก์ชันสำหรับใส่คอมม่าคั่นหลักพันและทศนิยม 2 ตำแหน่ง
function formatNumber(input) {
    let value = input.value.replace(/,/g, ''); // ลบคอมม่าก่อน
    if (!isNaN(value) && value !== '') {
        // แปลงเป็นเลขทศนิยม 2 ตำแหน่งและใส่คอมม่าคั่นหลักพัน
        input.value = parseFloat(value).toFixed(2).toLocaleString('en');
    } else {
