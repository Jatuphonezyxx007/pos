...
<!-- [ Header ] end -->
  ...
  <tbody>
    ...
    type.name AS type_name,  -- ใช้ชื่อคอลัมน์ที่ถูกต้องในตาราง type
    CASE 
...
FROM 
    `products`
JOIN 
    `size` ON products.id = size.product_id
JOIN 
    `type` ON products.type_id = type.type_id
WHERE 
    `products`.`id` = ?
GROUP BY 
    products.id, type.name  -- ใช้ชื่อคอลัมน์ที่ถูกต้องในตาราง type
ORDER BY 
    `products`.`type_id` ASC";

    // Prepare and execute the query
    if ($stmt = mysqli_prepare($conn, $sql)) {
        $product_id = isset($_GET['id']) ? $_GET['id'] : 0; 
        // ตรวจสอบว่ามีการส่งค่า id หรือไม่ และตั้งค่า default เป็น 0 ถ้าไม่มี
        mysqli_stmt_bind_param($stmt, "i", $product_id); // Assuming the product ID is an integer
    ...
    ?>
                <tr>
                    ...
                    <td class="text-center">
                        <img src="assets/images/products_2/<?=$data['id'];?>.<?=$data['img'];?>" class="card-img-top img-icon" alt="">
                    </td>
